# 读书搭子 · Buddy 应用配置准备（暂不推进）

本文件保留此前的 Buddy 应用控制台填写材料，**不是当前发布路径**。产品现已确定为“读书搭子专家 + 内置本地 Skill + 用户自己的阅读工作台”；因此不再为此文件配置 OAuth 回调、授权列表或提交 Buddy 应用。未来只有在需要独立品牌入口、多个工作模式与专属能力市场时再恢复评估。
