---
name: reading-buddy
description: 社交阅读平台 - 一起读书、交流心得。支持书目注册、虚拟读书室创建、实时聊天和心得分享。
openclaw:
  permissions:
    filesystem: true
    database: true
...
### 权限声明
- **文件系统访问**: 需要读写本地 SQLite 数据库文件，存储书目、用户和聊天数据
- **数据库操作**: 使用 `better-sqlite3` 进行数据持久化
- **网络访问**: 使用 WebSocket (`ws`) 提供实时聊天功能
- **进程间通信**: 无特权操作，仅标准 Node.js API

### 安全边界
- ✅ 数据本地存储，聊天数据不离开用户设备
- ✅ 使用标准 SQLite 数据库，无自定义二进制
- ✅ WebSocket 仅在本地运行，不连接外部服务器
- ✅ 代码开源可审计
- ❌ 不访问敏感系统文件
- ❌ 不执行特权操作

### 隐私保护
- 所有数据存储在 `~/.reading-buddy/` 目录下
- 聊天记录仅保存在本地数据库
- 不收集用户身份信息
- 不记录使用行为数据到外部服务
